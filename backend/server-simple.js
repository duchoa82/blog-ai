// server-simple.js - Simple fix for host parameter + App Bridge navigation
...
app.get('/auth', (req, res) => {
  ...
  const host = req.query.host; // Lưu host parameter
  ...
  req.session.oauthHost = host; // Lưu host để forward qua callback
...
app.get('/auth/callback', async (req, res) => {
  try {
    const { shop, hmac, state, code } = req.query;
    ...
    const oauthHost = req.session.oauthHost || 'admin.shopify.com'; // Fallback nếu không có host
    
    const adminUrl = `https://admin.shopify.com/store/${storeName}/apps/${appHandle}?host=${encodeURIComponent(oauthHost)}&shop=${encodeURIComponent(shop)}`;
